...
class DefaultController extends Controller
{
    ...
    {
        $dictionary = json_encode($this->getBaseDicctionaryArray());
    ...
    public function createAction(Request $request)
    {
        ...
        $hierarchy = json_decode($request->get('hierarchyStructure'), true);
        ...
        foreach ($hierarchy as $key => $item) {
            ...
            $hierarchyTransArray['METRIC_'.$key] = $request->get('trans_'.$key);
            if (is_array($item)) {
                foreach ($item as $key2 => $item2) {
                    $definitions[$key2] = 'METRIC_'.$key2;
                    $hierarchyTransArray['METRIC_'.$key2] = $request->get('trans_'.$key2);
                }
            }
        }
        ...
        $array = array(
            $clientName => array(
                ...
                'metrics' => $this->getMetricArray($definitions, $hierarchy),
                'region_metrics' => self::DELETE_ME,
                'themes' => self::DELETE_ME,
                'colors' => self::DELETE_ME,
                'country_codes' => self::DELETE_ME,
                'dictionary' => array(
                    'es' => array_merge(
                        $this->getBaseDicctionaryArray(),
                        $hierarchyTransArray
                    ),
                ),
            ),
            '_base' => $this->get_baseDicArray()
        );
    ...
    function getKeys($array)
    {
        ...
        foreach ($array as $key => $subArr) {
            if (is_array($subArr)) {
                $resultArr[$key] = array_values($subArr);
            } else {
    ...
    private function getMetricArray($definitions, $hierarchy)
    {
        $metric = array(
            'filter' => array(
                'GR' => 'FILTER_GR',
            ),
            'commentFilterField' => 'canal',
            'lowerIsBetter' => ['RP_TI'],
            'definitions' => $definitions,
            'hierarchy' => $this->getKeys($hierarchy),
            'competitors' => array(
            ...
            'ignore' => self::DELETE_ME,
        );

        return $metric;
    }

    private function getBaseDicctionaryArray()
    {
        $dictionary = array(
    ...
    private function get_baseDicArray()
    {
        $dictionary = array(
            'WELCOME' => 'Bienvenido',
            'DOWNLOAD_PDF' => 'Descargar PDF',
